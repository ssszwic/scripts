...
def parse_opt():
    ...
    parser.add_argument('--img_path', type=str, required=True, help='tif path')
    parser.add_argument('--save_path', type=str, required=True, help='new tif save path')
    parser.add_argument('--dpi', type=float, required=True, help='dpi')
    return parser.parse_args()

if __name__ == '__main__':
    args = parse_opt()
    src_path = args.img_path
    ...
    # cv2 resize
    ratio = im_Geotrans[1] / args.dpi
    im_data = cv2.resize(im_data, dsize=None, fx=ratio, fy=ratio, interpolation=cv2.INTER_LINEAR)
    im_data = np.transpose(im_data, (2, 0, 1)).astype(np.uint8)

    # 3. change Geotrans
    im_Geotrans = (im_Geotrans[0], args.dpi, im_Geotrans[2],im_Geotrans[3],im_Geotrans[4], args.dpi)
